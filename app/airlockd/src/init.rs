...
/// Spawn a background task that periodically reclaims sandbox
/// resources back to the host: `fstrim /mnt/disk` (sparse-image hole
/// punching) and `drop_caches=2` (kernel slab eviction, which lets
/// the host virtiofs proxy close cached FDs). Runs every 10 minutes
